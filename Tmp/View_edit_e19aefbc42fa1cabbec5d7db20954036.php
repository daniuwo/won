...
<div class="content openwindow">
    <form id="form" class="form-horizontal form-ajax" role="form" data-toggle="validator" method="POST" action="">
        ...
        <input type="hidden" name="row[rules]" value="<?php echo $ids;?>" />
        <input type="hidden" name="row[role_id]" value="<?php echo X('role_id');?>" />
        <input type="hidden" name="row[ud]" value="<?php echo X('ud');?>" />
        <div class="form-group">
            <label class="control-label col-xs-12 col-sm-2">头衔:</label>
            <div class="col-xs-12 col-sm-8">
                <input type="text" class="form-control" id="name" name="row[name]" value="<?php echo $data['name'];?>" data-rule="required" />
            </div>
        </div>
        <div class="form-group">
            <label class="control-label col-xs-12 col-sm-2">头衔颜色:</label>
            <div class="col-xs-12 col-sm-8">
                <input type="color" class="form-control" id="name" name="row[color]" value="<?php echo $data['color'];?>" data-rule="required" />
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-xs-12 col-sm-8">
                <div class="radio">
                    <label for="row[status]-normal"><input id="row[status]-normal" <?php if ($data['status'] == 1): ?>checked="checked"<?php endif ?> name="row[status]" type="radio" value="1"> 正常</label>
                    <label for="row[status]-hidden"><input id="row[status]-hidden" <?php if ($data['status'] == 0): ?>checked="checked"<?php endif ?> name="row[status]" type="radio" value="0"> 禁止</label>
                </div>
            </div>
        </div>
